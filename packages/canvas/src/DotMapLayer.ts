...
export class DotMapLayer extends Layer implements CanAnimation {
  ...
  private makeDots = (imgData: ImageData) => {
    const { pixelAndGapSize, stageHeight, stageWidth, stageY, stageX, pixelSize, gapSize, option } =
      this.canvasRenderer;
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < columns; j++) {
        ...
        if (data[pixelIndex] > 0 || data[pixelIndex + 1] > 0 || data[pixelIndex + 2] > 0) {
          dots.push(option.dotFactory.create(stageX + x, stageY + y, pixelSize, gapSize, option.pixelColor));
        }
      }
    }
